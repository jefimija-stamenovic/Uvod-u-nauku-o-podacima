...
Metrike: 
```{r}
mae_ridge = MAE(y_test, prediction_RF)
rmse_ridge = RMSE(y_test, prediction_RF)
r2_ridge = R2(y_test, prediction_RF)

cat("RF => MAE: ", mae_lasso, "\n")
cat("RF => RMSE: ", rmse_ridge, "\n")
cat("RF => R^2: ", r2_lasso, "\n\n")
```
...
ggplot(data, aes(x = actual, y = predicted)) +
  geom_point(color = "darkorange", alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1,  color = "red", linewidth = 1)+
    labs(title = "Stvarne vs. Predviđene vrednosti ()",
         x = "Stvarne vrednosti",
         y = "Predviđene vrednosti") +
  theme_minimal()
```

KNN ima najbolju adjusted R2 meru (iznosi 0.31) čime predstavlja najbolji model od svih prethodnih koji su testirani na ovom testnom skupu. 
